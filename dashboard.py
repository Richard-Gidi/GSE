"""
IC Securities Portfolio Analyser — ADVANCED EDITION (March 2026)
New features added:
• Primary live prices via official GSE-API[](https://dev.kwayisi.org/apis/gse/live) — fast JSON, no HTML scraping
• Automatic fallback to afx.kwayisi.org if API fails
• Portfolio Health Score (0-100) KPI with weighted formula
• Smart rebalance recommendations when HHI > 2500
• CSV export buttons for holdings + full transaction log
• One-click "Refresh Live Prices" button (clears cache + rerun)
• Minor UI polish and better error resilience
...
_LIGHT = SimpleNamespace(  # kept for future but not used
...
def apply_theme():
    """Inject full original CSS + tiny padding tweak."""
    p = th()
    is_dark = p.name == "dark"
    bg_pattern = (
...
section[data-testid="stSidebar"] {{
    background: rgba(22,25,41,0.97) !important;
...
/* INSIGHT BOXES, MOVER CARDS, SECTION HEADERS, CLIENT BAR, HERO, TABS, UPLOAD, DATAFRAMES, PLOTLY, EXPANDER, PILLS, LANDING, DIVIDER, POS/NEG, SIDEBAR, SCROLLBAR, SELECTION */
...
.sb-label {{ font-size: .65rem; color: {p.MUTED}; font-weight: 700; text-transform: uppercase; letter-spacing: .1em; margin-bottom: 10px; }}
.sb-note {{ font-size: .75rem; color: {p.MUTED}; line-height: 1.6; }}
...
# ─────────────────────────────────────────────────────────────────────────────
def _parse_gse_api(data: list, tickers: tuple) -> dict:
...
# ─────────────────────────────────────────────────────────────────────────────
# ALL CHARTS (original full — unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    with ct:
        st.markdown(
            ...
            f"Upload your statement · GSE-API live prices · Health score & smart rebalancing"
            f"</div>",
            unsafe_allow_html=True)
    with cr:
    ...
    if not uploaded:
        ...
        features = [
            ("📊", "Overview & KPIs", "Portfolio value, ROI, unrealised gains, Health Score & 8 KPI cards."),
            ("📈", "Performance Analysis", "Returns per stock, P&L waterfall, market vs cost, live price comparison."),
            ("⚖️", "Risk & Scenarios", "Break-even analysis, HHI gauge, smart rebalance suggestions, what-if simulator."),
            ("💸", "Cash Flow & Holdings", "Monthly flow charts, cumulative invested, full transaction log + CSV export."),
        ]
    ...
    if n_live == len(eq):
        st.success(f"📡 All {n_live} live prices from GSE-API")
    elif n_live:
    ...
    # Computed metrics + NEW Health Score & HHI
    ...
    if equities_val > 0:
        ...
        health_score = round(0.35*roi_score + 0.25*win_score + 0.20*div_score + 0.20*max(0, 100 - hhi/100))
    ...
    with tab1:
        ...
        with c4:
            health_color = GREEN if health_score >= 75 else AMBER if health_score >= 50 else RED
            st.markdown(kpi("Portfolio Health Score",
                            f"<span style='color:{health_color}'>{health_score}</span>/100",
                            "ROI + Win Rate + Diversification + Concentration",
                            "t" if health_score >= 75 else "y"), unsafe_allow_html=True)
    ...
    with tab3:
        ...
        st.markdown(
            ...
            f"Herfindahl-Hirschman Index (HHI): <b style='color:{rc}'>{hhi_val}</b> — "
        ...
        # NEW: Smart Rebalance Recommendations
        st.markdown("---")
        shdr("🛠️ Smart Rebalance Recommendations")
        if hhi_val > 2500:
            st.warning("⚠️ High concentration detected — consider trimming large positions")
            large_positions = sorted(
                [e for e in eq if e["market_value"]/equities_val > 0.12],
                key=lambda x: x["market_value"]/equities_val, reverse=True
            )[:3]
            for e in large_positions:
                wt = e["market_value"] / equities_val * 100
                st.info(f"• **Trim {e['ticker']}** (current weight {wt:.1f}% — target ~10%)")
        else:
            st.success("✅ Well diversified — no immediate rebalancing needed")

        st.markdown("---")
        shdr("🔮 What-If Scenario Simulator")
        st.caption("Drag sliders to simulate price moves")
    ...
    with tab5:
        ...
        with dl1:
            ...
            st.download_button("📥 Download Holdings as CSV",
                               csv_hold,
        ...
        with dl2:
            ...
            if not tx_df_export.empty:
                ...
                st.download_button("📥 Download Full Transaction Log CSV",
                                   csv_tx,
